...
{
    const handleSignOut=()=>{
        axios.get("http://localhost:3001/logout",axios.defaults.withCredentials = true
        ).then(res=>{
            if(res.status===200)
            {
   ...
   useEffect(()=>{
    ...
    <MainHeader ref={navbar} >
        ...
        <NavLink exact to="/" onClick={handleOff}  className="nav_link" activeClassName="active_navLink" >
            ...
            <NavLink onClick={handleOff} activeClassName="active_navLink" className="nav_link"  to="/product">
            <StoreIcon />SHOP
            </NavLink>
        ...
        :
        <NavLink onClick={handleProfileChange} activeClassName="active_navLink" className="nav_link1" to="#"><PersonIcon />{userdata.name.split(" ")[0]}</NavLink>
